...
export const getProducts = ( token,currentPage = 1, price = [0, 10000], category, ratings = 0) => async(dispatch) =>{
    try{
        dispatch({ type : Types.GET_PRODUCT_LOADING});

        let link =`${BASE_URL}/product/list`;
        ...
        // ?
        if(category){
          link = `${BASE_URL}/product/list?page=${currentPage}&price[gte]=${price[0]}&price[lte]=${price[1]}&category=${category}&ratings[gte]=${ratings}`
        }
